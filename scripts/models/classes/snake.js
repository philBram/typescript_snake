export class Snake {
    constructor(snakeStartPosition) {
        this.snakeBody = [snakeStartPosition];
    }
    moveSnake(directionUpdate) {
    ...
    isGameBoardCollision(gameBoardSize) {
    ...
    isSelfCollision() {
        return this.snakeBody.slice(1).some(item => item.x === this.snakeBody[0].x && item.y === this.snakeBody[0].y);
    }
    getSnakeBody() {
